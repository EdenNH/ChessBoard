...
function getAllPossibleCoordsRook(pieceCoord) {
    ...
    j = pieceCoord.j;
    for (idx = pieceCoord.i -1; idx > 0; idx++){
        var coord = { i: idx, j: j};
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
...
function getAllPossibleCoordsKnight(pieceCoord) {
    var res = [];
    for (var i = pieceCoord.i -1; i <= pieceCoord.i +1; i += 2){
        if (i < 8 && i > -1){
            for (var idx = pieceCoord.j -2; idx <= pieceCoord.j +2; idx += 4){
                if (idx < 8 && idx > -1){
                    var coord = {i: i, j:idx};
    ...
    for (var i = pieceCoord.i -2; i <= pieceCoord.i +2; i += 4){
        if (i < 8 && i > -1){
            for (var idx = pieceCoord.j -1; idx <= pieceCoord.j + 1; idx += 2){
                if (idx < 8 && idx > -1){
                    var coord = {i: i, j: idx};
                    if (isEmptyCell(coord)) 
                    res.push(coord);
...
function getAllPossibleCoordsKing(pieceCoord){
    ...
    for (var i = pieceCoord.i -1;  i<= pieceCoord.i +1; i++){
        if (i < 8 && i > -1){
            for (var idx = pieceCoord.j -1; idx <= pieceCoord.j +1; idx++){
                if (idx < 8 && idx > -1){
                    var coord = {i: i, j: idx};
                    if (isEmptyCell(coord)) 
                        res.push(coord)
...
function getAllPossibleCoordsQueen(pieceCoord){
    ...
    i = pieceCoord.i;
    for (idx = pieceCoord.j + 1; i >= 0 && idx < 8; idx++) {
        var coord = { i: i, j: idx };
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
    ...
    var j = pieceCoord.j;
    for (idx = pieceCoord.i + 1; j >= 0 && idx < 8; idx++) {
        var coord = { i: idx, j: j };
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
    j = pieceCoord.j;
    for (idx = pieceCoord.i - 1; j < 8 && idx >= 0; idx--) {
        var coord = { i: idx, j: j };
        if (!isEmptyCell(coord)) break;
        res.push(coord);
    }
